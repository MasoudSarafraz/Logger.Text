            ...
            var thread = new Thread(() =>
            {
                ...
                try
                {
                    for (int j = 0; j < logsPerThread; j++)
                    {
                        ...
                        {
                            case 0:
                                Logger2.LogInfo($"Thread-{threadId} Info message #{j}");
                                break;
                            case 1:
                                Logger2.LogStart($"Thread-{threadId} Start operation #{j}");
                                break;
                            case 2:
                                Logger2.LogEnd($"Thread-{threadId} End operation #{j}");
                                break;
                            case 3:
                                Logger2.LogRequest($"Thread-{threadId} Request data #{j}");
                                break;
                            case 4:
                                Logger2.LogResponse($"Thread-{threadId} Response data #{j}");
                                break;
                            case 5:
                                Logger2.LogError($"Thread-{threadId} Error simulation #{j}");
                                break;
                        }
                    }
                }
